# UI
admin-notes-title =  Notas para { $player }

admin-notes-new-note = Nova nota.

admin-notes-show-more =  Mostre mais.

admin-notes-for =  Nota para: { $player }

admin-notes-id = Id: { $id }

admin-notes-type =  Tipo: { $type }

admin-notes-severity = Severidade: { $severity }

admin-notes-secret =  Segredo

admin-notes-notsecret =  Não é segredo.

admin-notes-expires = Expira em: { $expires }

admin-notes-expires-never =  Não expira.

admin-notes-edited-never =  Nunca.

admin-notes-round-id = Id Redonda: { $id }

admin-notes-round-id-unknown = Id Redondo: Desconhecido

admin-notes-created-by =  Criado por: { $author }

admin-notes-created-at =  Criado em: { $date }

admin-notes-last-edited-by =  Última edição: { $author }

admin-notes-last-edited-at =  Última edição em: { $date }

admin-notes-edit =  Editar

admin-notes-delete =  Apagar

admin-notes-hide =  Esconda-se.

admin-notes-delete-confirm =  Confirmar deletar

admin-notes-edited =  Última edição por { $author } "Continue" { $date }

admin-notes-unbanned =  Não banida por { $admin } "Continue" { $date }

admin-notes-message-desc = [color=white] Você recebeu { $count ->
        [1]  Uma mensagem administrativa.
       *[other]  Mensagens administrativas
    }  Desde a última vez que jogou neste servidor.[/color]

admin-notes-message-admin = De [bold]{ $admin }[/bold], escrito em { TOSTRING($date, "f") }:

admin-notes-message-wait = O botão de aceitação será ativado depois. { $time }  segundos.

admin-notes-message-accept = Demitir permanentemente.

admin-notes-message-dismiss = Deem licença por enquanto.

admin-notes-message-seen =  Viu?

admin-notes-banned-from = Banida de

admin-notes-the-server = o servidor

admin-notes-permanently =  permanentemente

admin-notes-days = { $days }  dias

admin-notes-hours = { $hours }  horas

admin-notes-minutes = { $minutes }  minutos

# Note editor UI
admin-note-editor-title-new =  Criando uma nova nota para { $player }

admin-note-editor-title-existing = Editando nota { $id } "Continue" { $player } Por { $author }

admin-note-editor-pop-out =  Saia.

admin-note-editor-secret =  Segredo?

admin-note-editor-secret-tooltip =  Verificando isso, a nota não será visível pelo jogador.

admin-note-editor-type-note =  Nota

admin-note-editor-type-message =  Mensagem

admin-note-editor-type-watchlist =  Lista de observação

admin-note-editor-type-server-ban = Banimento do Servidor

admin-note-editor-type-role-ban = Proibição de papéis

admin-note-editor-severity-select =  Selecionar

admin-note-editor-severity-none =  Nenhum.

admin-note-editor-severity-low =  Baixo.

admin-note-editor-severity-medium =  Médio.

admin-note-editor-severity-high = Alto.

admin-note-editor-expiry-checkbox =  Permanente?

admin-note-editor-expiry-checkbox-tooltip =  Verifique isso para que expire.

admin-note-editor-expiry-label = Expira em:

admin-note-editor-expiry-label-params = Expira em: { $date } (em { $expiresIn })

admin-note-editor-expiry-label-expired = Expirado

admin-note-editor-expiry-placeholder =  Digite a data de validade (aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa

admin-note-editor-submit = Submeter

admin-note-editor-submit-confirm =  Tem certeza?

# Verb
admin-notes-verb-text =  Abra Notas de Administração

# Watchlist and message login
admin-notes-watchlist =  Lista de observação para { $player }: { $message }

admin-notes-new-message =  Você recebeu uma mensagem de administração de { $admin }: { $message }

admin-notes-fallback-admin-name = [System]

# Admin remarks
admin-remarks-command-description =  Abre a página de comentários do administrador.

admin-remarks-command-error = As observações da administração foram desativadas.

admin-remarks-title =  Observações da administração

# Misc
system-user = [System]
