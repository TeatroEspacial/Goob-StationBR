bwoink-user-title =  Mensagem de administração
...
admin-ahelp-admin-only-tooltip = Se marcada, então a mensagem não será visível para o jogador, mas será visível para outros administradores e ainda será transmitida Discord.

admin-bwoink-play-sound = Bwoink?

bwoink-title-none-selected =  Nenhum selecionado

bwoink-system-rate-limited = Sistema: você está enviando mensagens muito rápido.

bwoink-system-player-disconnecting =  Está desconectado.

bwoink-system-player-reconnecting = Reconectou-se.

bwoink-system-player-banned =  Foi banido por: { $banReason }

bwoink-message-admin-only = (Admin Apenas)
